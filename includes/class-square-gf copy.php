...
class Square_GF extends GF_Field
{
    ...
    public function add_payment_note_in_entry ( $note_args, $entry_id, $result ) {
        ...
        if ( $result === true && $transaction_id != '' ){  
            ...
            $note_args['text'] = sprintf(__( 'Payment has been completed. Amount %s %d, Transaction id: %s', 'gravity-forms-square' ), $currency, $amount, $transaction_id);
        }
        return $note_args;
    }
    ...
    public function add_card_details_in_entry ( $value, $field, $entry, $form ) {
        ...
        if ( $field->get_input_type() == 'square' && $card_type != '' ) { // Single file upload field
            ...
            $value = '<ul>
            <li>' . __( 'Card Type:', 'gravity-forms-square' ) . ' ' . $card_type . '</li>
            <li>' . __( 'Card Number:', 'gravity-forms-square' ) . ' ' . $card_num . '</li>
            <li>' . __( 'Card Exp:', 'gravity-forms-square' ) . ' ' . $card_exp . '</li>
            <li>' . __( 'Card Name:', 'gravity-forms-square' ) . ' ' . $card_name . '</li>
            </ul>';
    ...
    {
        add_action('gform_register_init_scripts', array($this, 'payment_scripts'), 10, 2);
    }
        ...
        foreach ($form['notifications'] as $notification_id => $notifications) {
            if (strpos($form['notifications'][$notification_id]['message'], '{square_payment_details}') !== false) {
                ...
                if (isset($entry['transaction_id']) && !empty($entry['transaction_id']) && isset($entry['payment_amount']) && !empty($entry['payment_amount'])) {
                    ...
                    $payment_details_email.='<tr><td '.$td_css.' >' . esc_html__( 'Payment Status', 'gravity-forms-square' ) . '</td><td '.$td_css.' >'.gform_get_meta($lead_ids[0], 'payment_status_returned').'</td></tr>';
    ...
    public function gform_entry_square_transaction_history($form, $entry) {
        ...
        if (isset($entry['payment_status']) && gform_get_meta($entry_id, 'payment_gateway') == 'squarerecurring') {
            ...
            $transactions = $wpdb->get_results("select * from $gfsr_transactions_table where entry_id = $entry_id order by created_at desc limit 10");
                ...
                try {
                    ...
                    if ($amount) {
                        if (isset($products['shipping']) && is_array($products['shipping'])) {
                            ...
                            //echo 'product shipping price is ' . $products['shipping']['price'];
                        }
                        ...
                        // Creating Order
                        //$order_id = $this->create_square_order($api_client, $location_id, $order_data);
                        ...
                        try {
                            ...
                            foreach ($errors as $error) {
                                ...
                                if (isset($error->field)) {
                                    $message = $error->field . ' - ' . $error->detail . '<br>';
                                }
                            }
                        ...
                        //$body->setReferenceId($_POST['orderID']);
                        ...
                        // echo '<pre>$transaction$transaction';
                        //     print_r($transaction);
                        // echo '</pre>';
                        
                        // wp_die('I am testing123!');
                        $transactionData = json_decode($transaction, true);

                        //echo json_encode(array('balance'=>20));
                        //wp_die();
                        ...
                        if (isset($transactionData['payment']['id'])) {
                            ...
                            echo json_encode(self::$transaction_response);
                        }
    ...
    public function payment_proccess($validation_result) { 
        ...
        foreach ($fields as $key => $field) {
            ...
            if ($field->type == 'square') {
                ...
                // before proceeding further we will check if user already paid with square gift card.
                // We will ignore all process and submit form simply
                if ( isset($_POST['transaction_response']) && isset($_POST['pay_with_giftcard']) && $_POST['pay_with_giftcard'] == 'yes' ) {
                    $validation_result['is_valid'] = true;
                    $form["fields"][$square_field_index]["failed_validation"] = false;
                    $validation_result['form'] = $form;
                    self::$transaction_response = json_decode(stripslashes($_POST['transaction_response']), true);
                    /*echo '<pre>$transaction_response$transaction_response';
                        print_r(self::$transaction_response);
                    echo '</pre>';*/
                    //wp_die('stop!!');
                    return $validation_result;
                }
        ...
        if ($is_square && $token && $location_id) {
            ...
            if ( 'enabled'===$cof_settings['square_cof_mode'] ) { 
            ...
            try {
                ...
                if ($amount) {
                   ...
                   // echo $note; exit;
                    ...
                    if ('enabled'===$is_recurring || $is_multiple_square == 'enabled' ) { 
                    ...
                    //for simple payments
                    if ('disabled'===$is_recurring && $is_multiple_square == 'disabled') { 
    ...
    public function payment_scripts($form) {
        ...
        if (isset($settings['gf_squaree_mode']) && $settings['gf_squaree_mode'] == 'test') {
            ...
            $application_id = $settings['square_appid'];
            $location_id = $settings['square_locationid'];
        }
    ...
    {
         wp_enqueue_style('gfsq-style-admin', SQGF_PLUGIN_URL . 'assets/style/admin-style.css');
    }
    ...
    public function get_field_input($form, $value = '', $entry = null)
    {
        ...
        if (is_user_logged_in() && $settings && !empty($settings['square_cof_save_card_text']) && 'enabled'==$settings['square_cof_mode']) {
            ...
            if (!is_admin()) {
                        ...
                        if (isset($square_customer['customer']['cards'])) {
                            ...
                            foreach ($square_customer_cards as $key => $card) {
                                ...
                                if ( 'enabled'===$square_cof_delete_card ) {
                                    $delete_card = '<div class="gfsqs-trigger-btn"><span class="dashicons dashicons-trash"></span></div>';
                                }
                ...
                if (!$is_customer_found) {
                    $radio_for_square_rendered_fields = '<input type="radio" id="toggler_2_' . $form_id . '" class="gf_square_payment_toggler" data-id="new_card_paymment" name="gf_square_payment_toggler" value="gf_sqquare_container_' . $form_id . '" checked >';
                }
